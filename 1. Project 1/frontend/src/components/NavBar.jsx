...
const NavBar = () => {
  ...
  return (
    <div className='flex items-center justify-between py-5 font-medium'>
        <Link to='/'>
            <img src={assets.logo} className='w-36' alt="ShopEZ" />
        </Link>
        <ul className='hidden gap-5 text-sm text-on-bg sm:flex'>
        ...
        <div className='flex items-center gap-6'>
            ...
            <div className='relative group'>
                ...
                {token && (
                    <div className='absolute right-0 hidden pt-4 group-hover:block dropdown-menu'>
                        <div className='flex flex-col gap-2 px-5 py-3 text-muted rounded w-36 bg-white shadow-lg border'>
                            <p onClick={() => navigate('/orders')} className='cursor-pointer hover:text-brand'>My Orders</p>
                            <p onClick={logout} className='cursor-pointer hover:text-brand'>Logout</p>
                        </div>
                    </div>
                )}
            </div>
            <Link to='/cart' className='relative'>
                ...
                <p className='absolute right-[-5px] bottom-[-5px] w-4 text-center leading-4 bg-brand text-white aspect-square rounded-full text-[8px]'>{getCartCount()}</p>
            </Link>
